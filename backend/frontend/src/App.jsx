...
import { Plus, Trash2, Download, Play, Save, CheckCircle, XCircle, Loader, Clock, Eye, Edit } from 'lucide-react';
                  ...
                  <div key={pb.id} className="p-4 border rounded hover:bg-gray-50 transition">
                    <div className="flex justify-between items-start">
                      <div className="flex-1 cursor-pointer" onClick={() => loadPlaybookDetail(pb.id)}>
                        ...
                        <p className="text-xs text-gray-500">{pb.id}</p>
                      </div>
                      <div className="flex gap-2">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white rounded-lg shadow-lg p-6">
              ...
              <div className="mb-6">
                ...
                <input type="text" value={inventory.name} onChange={(e) => setInventory({ ...inventory, name: e.target.value })} className="w-full px-3 py-2 border rounded mb-4" />
                
                <label className="block text-sm font-medium mb-2">Inventory Content (INI format)</label>
                <textarea value={inventory.content} onChange={(e) => setInventory({ ...inventory, content: e.target.value })} rows={12} className="w-full px-3 py-2 border rounded font-mono text-sm" />
...
localhost ansible_connection=local`}
                ...
                <button onClick={saveInventory} className="mt-4 px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">Save Inventory</button>
              </div>
              <h3 className="text-xl font-semibold mb-4">Saved Inventories</h3>
              <div className="space-y-2">
                {inventories.map(inv => (
                  <div key={inv.id} className="p-4 border rounded hover:bg-gray-50 transition">
                    <div className="flex justify-between items-center">
                      <div className="flex-1 cursor-pointer" onClick={() => loadInventoryDetail(inv.id)}>
                        ...
                        <p className="text-sm text-gray-600">{inv.id}</p>
                      </div>
                      <div className="flex gap-2">
